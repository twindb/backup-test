...
.PHONY: build
build: ## Build image.
	@echo "Building image tagged as twindb/backup-test:$$(git branch --no-color --show-current)"
	docker build -t "twindb/backup-test:$$(git branch --no-color --show-current)" .


.PHONY: publish
publish: ## Publish image to hub.docker.com
	@echo "Publishing image tagged as twindb/backup-test:$$(git branch --no-color --show-current)"
